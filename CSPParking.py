...
def arg_parse(arg_dimensions: str, arg_electric: str, arg_ambulances:list) -> Tuple[list, list,list]:
    ...
    arg_ambulances = [i.rstrip() for i in arg_ambulances]
...
class grid:
    ...
    def __init__(self, n, m, electric_values):
        ...
        for grid_elect_position in electric_values:
            self.grid[grid_elect_position[1]-1][grid_elect_position[0]-1] = 'E'
...
for i in range(len(ambulances)):
    problem.addVariable(ambulances[i],range(grid.getn()*grid.getm())) #We now consider the problem space of the grid to be a singular array of integers which (1,1) will correspond to 1 and (1,2) to 2 so on (2,1) to 6 in our case