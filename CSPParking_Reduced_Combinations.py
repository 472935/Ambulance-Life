...
for ambulance1 in ambulance_vect:
    for ambulance2 in ambulance_vect:
        if ambulance2.urgent == "S" and ambulance1.urgent == "N":
            problem.addConstraint(tsu_row, (ambulance2.id, ambulance1.id))
        else:
            if ambulance1.charger == ambulance2.charger and int(ambulance1.id) < int(ambulance2.id): #With previous else, forces ambulances of same urgency and chrger type to be placed eith increasing id order to reduce the number of combinations of solutions
